/*
 * KMagneticFastMultipoleFieldSolver.cc
 *
...
#include "KFMMagnetostaticTreeConstructor.hh"
...
KMagneticFastMultipoleFieldSolver::KMagneticFastMultipoleFieldSolver() :
    ...
    fUseOpenCL(false)
{
    fTree = new KFMMagnetostaticTree();
}
...
void KMagneticFastMultipoleFieldSolver::InitializeCore(KSurfaceContainer& container)
{
    kfmout << GetParameterInformation() << kfmendl;

    //the tree constuctor definitions
    using TreeConstructor_SingleThread = KFMMagnetostaticTreeConstructor<KFMMagnetostaticFieldMapper_SingleThread>;
#ifdef KEMFIELD_USE_OPENCL
    typedef KFMMagnetostaticTreeConstructor<KFMMagnetostaticFieldMapper_OpenCL> TreeConstructor_OpenCL;
...
#else
    using TreeConstructor_OpenCL = KFMMagnetostaticTreeConstructor<KFMMagnetostaticFieldMapper_SingleThread>;
#endif
    ...
    string fmContainerBase(KFMMagnetostaticTreeData::Name());
    ...
    auto* tree_data = new KFMMagnetostaticTreeData();
    ...
    if (fUseOpenCL) {
        ...
        fFastMultipoleFieldSolverOpenCL =
            new KFMMagnetostaticFastMultipoleFieldSolver_OpenCL(fIntegratorPolicy.CreateOpenCLConfig(),
                                                                *oclContainer,
                                                                *fTree);
        return;
#endif
    ...
    fFastMultipoleFieldSolver = new KFMMagnetostaticFastMultipoleFieldSolver(container, *fTree);
    return;
}

double KMagneticFastMultipoleFieldSolver::PotentialCore(const KPosition& P) const
{
    if (fUseOpenCL) {
#ifdef KEMFIELD_USE_OPENCL
...
KFieldVector KMagneticFastMultipoleFieldSolver::MagneticFieldCore(const KPosition& P) const
{
    if (fUseOpenCL) {
#ifdef KEMFIELD_USE_OPENCL
        return fFastMultipoleFieldSolverOpenCL->MagneticField(P);
#endif
    }
    return fFastMultipoleFieldSolver->MagneticField(P);
}

std::string KMagneticFastMultipoleFieldSolver::GetParameterInformation()
{
...
void KMagneticFastMultipoleFieldSolver::UseOpenCL(bool choice)
{
    if (choice == true) {
#ifdef KEMFIELD_USE_OPENCL
        fUseOpenCL = choice;
...
void KMagneticFastMultipoleFieldSolver::SetSplitMode(bool choice)
{
